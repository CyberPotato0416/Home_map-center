...
> ⚠️ **【部分已更新】** 本文件的 **通勤計分公式**（`Commute Score = max(0, 100 - (distToOffice/100) - (distToMrt/20))`）已由 **Phase 7 規格書** 及 **`src/utils.ts`** 取代。  
> 目前實際執行的通勤力進度欄公式為：  
> `commuteScore = distKm <= 1.0 ? 10 : 10 - ceil((distKm - 1.0) / 0.5)`  
> 其餘 UI 設計內容（屬性網格、內容面板結構）仍有參考價値。
